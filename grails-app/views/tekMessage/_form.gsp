...
<div class="fieldcontain ${hasErrors(bean: tekMessageInstance, field: 'author', 'error')} ">
	<label for="author">
		<g:message code="tekMessage.author.label" default="Author" />
		
	</label>
	<g:select id="author" name="author.id" from="${com.tekdays.TekUser.list()}" optionKey="id" value="${tekMessageInstance?.author?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
